...
RESULTS_DIR="/raid/petera/genesis/results/npe-oracle-combined/$(date -Iminutes)"
mkdir -p $RESULTS_DIR
...
function start_dl() {
	echo "scp -r ${MACHINES[$1]}:/home/ubuntu/Workspace/genesis/rdir$2 $RESULTS_DIR/ >/tmp/scp_rdir$2_out.log 2>/tmp/scp_rdir$2_err.log &" >/tmp/scp_rdir$2_cmd
	scp -r ${MACHINES[$1]}:/home/ubuntu/Workspace/genesis/rdir$2 $RESULTS_DIR/ >/tmp/scp_rdir$2_out.log 2>/tmp/scp_rdir$2_err.log
	echo "scp -r ${MACHINES[$1]}:/home/ubuntu/Workspace/genesis/wdir$2 $RESULTS_DIR/ >/tmp/scp_wdir$2.log 2>/tmp/scp_wdir$2_err.log &" >/tmp/scp_wdir$2_cmd
	scp -r ${MACHINES[$1]}:/home/ubuntu/Workspace/genesis/wdir$2 $RESULTS_DIR/ >/tmp/scp_wdir$2.log 2>/tmp/scp_wdir$2_err.log
}
...
function queue_case() {
	case=${CASES_TO_RUN[0]}
	ssh ${MACHINES[$1]} tmux new-session -d -s genesis_repair_$case "'cd /home/ubuntu/Workspace/genesis; bash/run-combined-case.sh $case oracle'"
