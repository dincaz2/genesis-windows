...
RESULTS_DIR="/raid/petera/genesis/results/npe-oracle/$(date -Iminutes)"
mkdir -p $RESULTS_DIR
...
function queue_case() {
	case=${CASES_TO_RUN[0]}
	ssh ${MACHINES[$1]} tmux new-session -d -s genesis_repair_$case "'cd /home/ubuntu/Workspace/genesis; bash/run-case.sh $case oracle'"
