...
class maven_testinfo:
    def __init__(self, repo_dir):
        ...
        ori_dir = getcwd();
        chdir(repo_dir);
        p = subprocess.Popen('cmd /c "mvn -fn -X test"', stdout=subprocess.PIPE);
        chdir(ori_dir);
        ...
        for line in lines:
            ...
            if (len(tokens) == 0):
                continue;
            if (len(tokens) > 3) and (tokens[0] == "[INFO]") and (tokens[1] == "---") and (
                    tokens[2].find("maven-surefire-plugin") == 0):
                if tclass != None:
                ...
                sdir = ""
                cpath = [];
            elif intest and (((len(tokens) >= 5) and (tokens[0] == 'T') and (tokens[1] == 'E') and (
                    tokens[2] == 'S') and (tokens[3] == 'T') and (tokens[4] == 'S')) or (
                                     (len(tokens) >= 6) and (tokens[1] == 'T') and (tokens[2] == 'E') and (
                                     tokens[3] == 'S') and (tokens[4] == 'T') and (tokens[5] == 'S'))):
                intest = False;
                ...
                self.num_module += 1;
                tclass = [];
            elif intest and (len(tokens) > 4) and ((tokens[0] == "[INFO]") or (tokens[0] == "[DEBUG]")) and (
                    tokens[1] == "Surefire") and (tokens[2] == "report") and (tokens[3] == "directory:"):
                rdir = tokens[4];
                ...
                tcdir = tokens[4];
                    ...
                    ((len(tokens) > 3) and (tokens[1] == "test") and (tokens[2] == "classpath") and (
                            tokens[3] == "classpath:") or \
                     (len(tokens) > 3) and (tokens[1] == "Test") and (tokens[2] == "Classpath") and (
                             tokens[3] == ":") or \
                ...
                if (len(tokens) == 2) and (tokens[0] == "[DEBUG]") and (tokens[1] != "provider"):
                    ...
                    incp = False;
            elif (len(tokens) == 3) and (tokens[1] == "Running"):
                tclass.append(tokens[2]);
    ...
    def writeto(self, fout):
        ...
        for i in range(0, self.num_module):
            print >> fout, self.format_slash(self.base_dir[i])
            print >> fout, self.format_slash(self.report_dir[i])
            print >> fout, self.format_slash(self.class_dir[i])
            print >> fout, self.format_slash(self.testclass_dir[i])
            print >> fout, self.format_slash(";".join(self.classpath[i]))
            print >> fout, self.format_slash(self.source_dir[i])
            print >> fout, len(self.testclass[i]),
            for tclass in self.testclass[i]:
                print >> fout, self.format_slash(tclass),
            print >> fout
            assert (len(self.testclass[i]) == len(self.testcase[i]));
            for testcases in self.testcase[i]:
                print >> fout, len(testcases)
                for case in testcases:
                    print >> fout, self.format_slash(case[0]), self.format_slash(case[1]), self.format_slash(case[3])

    def format_slash(self, text):
        return text.replace('\\', '/') if isinstance(text, basestring) else text
