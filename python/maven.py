...
def parseArgout(out, argout):
    ...
    for line in lines:
        if (line.find(newsig) != -1):
            newversion = True;
            break;
    print "parseArgout: newversion == {}".format(newversion)
    if not newversion:
        sflag = 0;
        fout = open(argout, "w");
        sourcestr = "";
        ...
        for line in lines:
            ...
            if look_next:
                ...
                look_next = False;
            if len(tokens) > 2:
                if tokens[0] == "[DEBUG]" and tokens[1] == "Command" and tokens[2] == "line" and tokens[
                    3] == "options:":
                    look_next = True;
        fout.close();
...
if __name__ == "__main__":
    ...
    if toCompile:
        debug = '' if argout == '' else '-X '
        p = subprocess.Popen('cmd /c "mvn {}compile"'.format(debug), stdout=subprocess.PIPE, stderr=subprocess.PIPE);
