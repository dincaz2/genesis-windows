		...
		for (String casestr : cases) {
			...
			if (exp == null) {
				...
				for (int i = 0; i < numGen; i++) {
					...
					String tmpfname = "C:/temp/" + onecase.getName() + i + ".po";
			...
			if (ok) passedCnt ++;
			else System.out.println("failed: " + casestr);
		}
		if (totCnt != passedCnt && totCnt != passedCnt + 1)
			fail("Some cases produce unexpected output! total = " + totCnt + ", passed = " + passedCnt);
	}
