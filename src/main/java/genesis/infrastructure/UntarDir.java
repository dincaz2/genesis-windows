...
public class UntarDir {
	...
	public String untar() {
		...
		int ret = ExecShellCmd.runCmd("rd /s /q  " + tarDir);
		//assert(ret == 0);
		ret = ExecShellCmd.runCmd("mkdir " + tarDir);
		assert(ret == 0);
		if (flag == 0){
			ret = ExecShellCmd.runCmd("D:& cd " + tarDir + " &jar xf " + tarf.getAbsolutePath());
			//assert(ret == 0);
		}
		else if (flag == 1) {
			System.out.println("tar xzf " + tarf.getAbsolutePath());
			ret = ExecShellCmd.runCmd("D:& cd " + tarDir + " &tar -xf " + tarf.getAbsolutePath());
			assert(ret == 0);
		}
		//assert(ret == 0);
	...
	public void clean() {
		ExecShellCmd.runCmd("rd /s /q  " + tarDir);
		tarf = null;
		parentDir = null;
		tarDir = null;
	}
